...
class StaticGraphLSTMCell_(Module):
    def __init__(self, input_size: int, hidden_size: int, num_nodes: int = None, dropout: float = 0.,
                 ...
                 weights_per_type: bool = False, bias: bool = True):
        """
    ...
    def forward(self, input: torch.Tensor, state: GraphLSTMState) -> Tuple[torch.Tensor, GraphLSTMState]:
        hx, cx, gx = state
        if hx is None:
...
class StaticGraphLSTM_(Module):
    ...
    def forward(self, input: torch.Tensor, states: Optional[List[GraphLSTMState]] = None) -> Tuple[torch.Tensor, List[GraphLSTMState]]:
        if states is None:
        ...
        for rnn_layer in self.layers:
            ...
            for input in inputs:
                out, state = rnn_layer(input, state)
                outputs += [out]
