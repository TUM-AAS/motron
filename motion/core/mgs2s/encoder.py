...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.rnn = StaticGraphLSTM(input_size, output_size, num_layers=1, num_nodes=21, node_types=node_types, dropout=0.0, clockwork=True)
        self.fc = StaticGraphLinear(output_size, output_size, num_nodes=21, node_types=node_types)
        self.initial_hidden1 = StaticGraphLinear(input_size, output_size, num_nodes=21, node_types=node_types)
        self.initial_hidden2 = StaticGraphLinear(input_size, output_size, num_nodes=21, node_types=node_types)
    ...
    def forward(self, x: torch.Tensor, state: torch.Tensor = None) -> torch.Tensor:
        ...
        return self.dropout(torch.tanh(self.fc(y[:, -1]))), state
