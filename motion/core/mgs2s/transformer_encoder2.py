...
class MyTransformerEncoder(Module):
    def __init__(self,
        ...
        encoder_layers0 = TransformerEncoderLayer(d_model=hidden_size+4, nhead=2, dropout=0.0)
        self.transformer_encoder0 = TransformerEncoder(encoder_layers0, 2)


        encoder_layers = TransformerLayer(d_model=output_size, nhead=4, dropout=0.0)
        self.transformer_encoder = TransformerEncoderML(encoder_layers, 3)
        self.fc = Linear(in_features=input_size, out_features=hidden_size)
        self.fc1 = Linear(in_features=hidden_size+4, out_features=output_size)
        self.dropout = Dropout(0.0)

        self.pe = PositionalEncoding(4)

        self.register_buffer('mask', self.generate_square_subsequent_mask(50))

    def generate_square_subsequent_mask(self, sz):
        # mask = (torch.triu(torch.ones(sz, sz)) == 1).transpose(0, 1)
        # mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        # return mask
        mask = torch.ones(sz, sz) * True
        for i in range(sz):
            mask[i, torch.max(torch.tensor([i-10, 0])):i+1] = False
        return mask

    def forward(self, x, state=None):
        bs, ts, ns, fs = x.shape
        x = self.fc(x)
        x_rshp = x.permute(1, 0, 2, 3).reshape(ts, -1, self.hidden_size)
        all = self.pe(x_rshp)
        y = self.transformer_encoder0(all, self.mask)
